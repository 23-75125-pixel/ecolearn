    ...
    <Card className="mt-8">
      ...
      <form action={formAction} className="space-y-6" encType="multipart/form-data">
        ...
        <fieldset className="space-y-4">
          ...
          <div><span className="mb-2 block text-sm font-medium text-foreground">Subjects <span className="text-danger">*</span></span>{subjects.length === 0 ? <p className="rounded-md bg-danger/10 p-3 text-sm text-danger">No subjects are available. Please ask an administrator to add subjects first.</p> : <div className="grid gap-2 rounded-md border border-border bg-surface p-3 sm:grid-cols-2">{subjects.map((subject) => <label key={subject.id} className="flex min-h-10 items-center gap-2 text-sm text-foreground"><input type="checkbox" name="subjectIds" value={subject.id} defaultChecked={selected.has(subject.id)} className="h-4 w-4 accent-primary-600" />{subject.name}</label>)}</div>}</div>
